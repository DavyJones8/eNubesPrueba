...
class Auth extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->language('auth_lang','spanish');
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if($inputCaptcha === $sessCaptcha){
				...
				$this->data['message_error_captcha'] = 'El captcha introducido no es correcto';
			}
		...
		if ($this->form_validation->run() === TRUE && $flag_captcha && $this->ion_auth->register($identity, $password, $email, $additional_data))
		{
			// check to see if we are creating the user
			// redirect them back to the admin page
			$this->session->set_flashdata('message', 'Registro completado');
			...
			$config['smtp_user']    = 'noreply@enubesprueba.ignaciovalero.es';
			$config['smtp_pass']    = '********';
			...
			$to_email_1 = $this->input->post('email');
			$subject = 'Mensaje de bienvenida';
			$data_template['nombre']    = $this->input->post('first_name');
			$data_template['apellidos'] = $this->input->post('last_name');
		
			$this->email->set_newline("\r\n");
			$this->email->from('noreply@enubesprueba.ignaciovalero.es', 'eNubes News');
			$this->email->to($to_email_1);
			$this->email->subject($subject); 
		
			$body = $this->load->view('templates_email/bienvenida.php',$data_template,TRUE);
			...
			if( !$this->email->send() ){
				...
				if( !$this->email->send() ){
					...
					$this->session->set_flashdata('message_error', 'No se pudo enviar el email de bienvenida.');
